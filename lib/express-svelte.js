...
function expressSvelte(opts = {}) {
    ...
    async function svelte(name, renderOptions = {}) {
        ...
        try {
            ...
            const scriptLegacy = scriptSrc ? `${bundlesHost}/${scriptSrc}`.replace('.js', '-legacy.js') : null;
            const scriptModern = scriptSrc ? `${bundlesHost}/${scriptSrc}`.replace('.js', '-modern.js') : null;
