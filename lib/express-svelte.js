...
function expressSvelte(mainOpts = {}) {
    ...
    async function svelteCompile(name, options = {}) {
        try {
            ...
            try {
                ...
                const { default: WrappedViewComponent, load = null } = await ServerViewFactory.createWrappedViewAndCompile(filename, compileOptions);
    ...
    async function svelteRender(name, renderOptions = {}) {
        ...
        try {
            // Create view component wrapped with ViewGlobals.svelte component and compile
            const { WrappedViewComponent, load, filenameKey, cache, hydratable } = await svelteCompile(name, renderOptions);

            // Default options
            renderOptions.props = renderOptions.props || {};
            renderOptions.globalProps = renderOptions.globalProps || {};
            renderOptions.globalStores = renderOptions.globalStores || {};

            // Shadow copy props to avoid modifications during rendering
            ...
            try {
                if (load != null) {
                    await load({
                        ...
                        globalProps: renderOptions.globalProps,
                        globalStores: renderOptions.globalStores
                    });
                }
