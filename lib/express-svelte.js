...
function expressSvelte(opts = {}) {
    ...
    async function svelte(name, options = {}) {
        ...
        try {
            ...
            try {
                ViewComponent = await ViewCompiler.compile(viewFilename, opts);
            }
            catch(err) {
            ...
            const scriptModern = scriptSrc ? `${bundlesHost}/legacy${scriptSrc}` : null;
 ...
 *
 * @property {Boolean}   [legacy = true]       Defaults to true.
 *
