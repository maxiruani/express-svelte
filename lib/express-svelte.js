    ...
    return function expressSvelteMiddleware(req, res, next) {
        ...
        res.svelte = async function svelte(name, options = {}) {
            try {
                ...
                // TODO: Improve devalue of props that are not present on template

                const str = templateString
                        .replace('${head}', head)
                        .replace('${styles}', `<style type="text/css">${css.code}</style>`)
                        .replace('${globalProps}', `<script type="text/javascript">window._GLOBAL_PROPS_ = ${devalue(serializedGlobalProps)};</script>`)
                        .replace('${globalStore}', `<script type="text/javascript">window._GLOBAL_STORE_ = ${devalue(serializedGlobalStore)};</script>`)
                        .replace('${props}', `<script type="text/javascript">window._PROPS_ = ${devalue(componentProps)};</script>`)
