...
// Wrapper creation
let _tmpDirname = null;
let _wrapperViewCacheMap = new Map();
...
class ServerViewFactory {
    ...
    static clear() {
        // Wrapper creation
        _wrapperViewCacheMap = new Map();
        _tmpDirname = process.cwd() + '/.express-svelte/server';
        this.clearAndEnsureDirSync(_tmpDirname);
    ...
    static async create(filename) {
        ...
        let viewComponentFilename = _wrapperViewCacheMap.get(filename) || null;
        ...
        viewComponentFilename = _tmpDirname + '/' + crypto.randomBytes(10).toString('hex') + '.svelte';

        await fs.writeFile(viewComponentFilename, viewComponentContent, { encoding: 'utf8', flag: 'w' });
        _wrapperViewCacheMap.set(filename, viewComponentFilename);

        return viewComponentFilename;
    }

    /**
     * @param {String} wrapperViewFilename
     * @param {ExpressSvelteCompileOptions} [opts]
     * @return {Promise.<Function>}
     */
    static async compile(wrapperViewFilename, opts = {}) {
        ...
        if (cache === true) {
            ...
            cacheKey = `${wrapperViewFilename}|hydratable:${hydratable}`;
        ...
        if (output.length !== 1) {
            const err = new Error(`Invalid compile output. Generated more than one chunk or asset. wrapperViewFilename:${wrapperViewFilename} output.length:${output.length}`);
        ...
        if (chunk.type !== 'chunk') {
            const err = new Error(`Invalid compile output. Generated an asset instead of a chunk. wrapperViewFilename:${wrapperViewFilename}`);
     ...
     * @return {Promise.<Function>}
     */
    static async createAndCompile(filename, opts) {
        const wrapperViewFilename = await this.create(filename);
        return this.compile(wrapperViewFilename, opts);
    }
}
