...
class BrowserViewFactory {
     ...
     */
    static async generateCompleteSource(absoluteFilename, relativeFilename) {

        const viewGlobalsRelativePath = path.relative(path.dirname(absoluteFilename), __dirname + '/components') + '/ViewGlobals.svelte';
...
new ViewGlobals({
    ...
    static async createWrappedView(absoluteFilename, output, hydrationMode) {
        ...
        if (hydrationMode === this.HydrationMode.PARTIAL) {
            ...
            source = await this.generateCompleteSource(absoluteFilename, relativeFilename);
        }
