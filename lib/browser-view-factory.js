...
const DEV = NODE_ENV === NODE_ENV_DEVELOPMENT || NODE_ENV === NODE_ENV_TESTING;
...
class BrowserViewFactory {
    ...
    static async getAppEnvVirtualModule(env) {
        ...
        .replace('{{APP_ENV_BROWSER}}', 'true')
        .replace('{{APP_ENV_SERVER}}', 'false')
        .replace('{{APP_ENV}}', env)
    ...
    static async generateSource(relativeFilename, wrappedAbsoluteFilename) {
        ...
        const viewGlobalsRelativePath = path.relative(path.dirname(wrappedAbsoluteFilename), __dirname + '/components') + '/View.svelte';

        return `
import View from '${viewGlobalsRelativePath}';
import ViewComponent, * as ViewComponentStatic from '${relativeFilename}';
...
if (ViewComponentStatic.load != null) {
    await ViewComponentStatic.load({ props, globalProps, globalStores });
}
...
new View({
    ...
    static async getConfig(opts = {}) {
        ...
        const appEnvModule = await this.getAppEnvVirtualModule(NODE_ENV);
    ...
    static async create(input, opts = {}) {
        ...
        for (let i = 0; i < absoluteFilenames.length; i++) {
            ...
            let completeLegacyConfig = await this.getConfig({ ...opts, input: wrappedAbsoluteFilename, output: `${output}-modern`, legacy: false });
            configs.push(completeLegacyConfig);

            // Push legacy config
            if (legacy === true) {
                wrappedAbsoluteFilename = await this.createWrappedView(absoluteFilename, output);
                completeLegacyConfig = await this.getConfig({ ...opts, input: wrappedAbsoluteFilename, output: `${output}-legacy`, legacy: true });
                configs.push(completeLegacyConfig);
            }
        }
