...
import { ViewGlobals } from 'express-svelte';
...
new ViewGlobals({
    ...
    static async createWrappedView(absoluteFilename, output, hydrationMode) {
        ...
        if (hydrationMode === this.HydrationMode.PARTIAL) {
            // source = await this.generatePartialSource(input, relativeFilename);
        }
        else {
            source = await this.generateCompleteSource(relativeFilename);
        }
    ...
    static getConfig(opts = {}) {
        ...
        return {
            ...
            input: {
                [output + `${legacy ? 'legacy' : 'modern'}`]: input
            },
            output: {
                sourcemap: DEV,
                format: 'iife',
    ...
    static async create(input, opts = {}) {
        ...
        for (let i = 0; i < absoluteFilenames.length; i++) {
            ...
            const wrappedAbsoluteFilename = await this.createWrappedView(absoluteFilename, output, this.HydrationMode.COMPLETE);
            const completeLegacyConfig = this.getConfig({ ...opts, input: wrappedAbsoluteFilename, output, legacy: false });
