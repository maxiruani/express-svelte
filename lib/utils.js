...
const PATTERN_TYPE = /-(?:complete|partial)-(?:legacy|modern).js/;
...
class Utils {
    ...
    static getFilenamesMaps(filenames, pattern, roots) {
        ...
        for (let i = 0; i < filenames.length; i++) {
            ...
            const PATTERN_LEGACY = '123123123123';

            const key = path.join(dirname, '/' + name).replace(PATTERN_LEGACY, '/');
            const value = relative.replace(PATTERN_LEGACY, '/');
