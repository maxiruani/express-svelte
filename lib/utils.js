...
class Utils {
    ...
    static getRelativeFilename(filename, roots) {
        ...
        for (let i = 0; i < roots.length; i++) {
            const root = roots[i];
            relative = filename.replace(new RegExp('^' + _.escapeRegExp(root[i])), '');
        }
        return relative;
    }
    ...
    static getFilenamesMaps(filenames, pattern, roots) {
        ...
        for (let i = 0; i < filenames.length; i++) {
            ...
            if (result == null) {
                restMap.set(dirname + '/' + basename, absolute);
                continue;
            }
            ...
            if (extname === '.js') {
                scriptsMap.set(dirname + '/' + name, absolute);
            }
            else {
                stylesMap.set(dirname + '/' + name, absolute);
            }
        }
     ...
     * @return {String}
     */
    static async getTemplate(filename, opts) {
        ...
        let template = cache === true ? _cacheMap.get(filename) : null;

        if (template != null) {
            return template;
        }

        template = fs.readFile(filename, { encoding: 'utf-8' });

        if (cache === true) {
            _cacheMap.set(filename, template);
        }
