...
class Utils {
    ...
    static getFilenamesMaps(filenames, pattern, roots) {
        ...
        for (let i = 0; i < filenames.length; i++) {
            ...
            const key = path.join(dirname, '/' + name);
            const value = relative;

            if (extname === '.js') {
                scriptsMap.set(key, value);
            }
            else {
    ...
    /**
     * @param {String} filename
     * @param {ExpressSvelteTemplateOptions} [opts]
     * @return {String}
     */
     ...
     * @return {Boolean}
     */
    static isValidHydrationMode(value) {
     ...
     * @return {Boolean}
     */
    static isCompleteHydrationMode(value) {
     ...
     * @return {Boolean}
     */
    static isPartialHydrationMode(value) {
        return value === 'partial';
    }

    /**
     * @param {String|Boolean} value
     * @param {String|Boolean} def
     * @return {String|Boolean}
     */
    static sanitizeHydrationMode(value, def) {
        if (value == null) {
            return def;
        }
        if (value === false || value === 'complete') {
            return value;
        }
        throw new Error('HYDRATION_MODE_NOT_SUPPORTED');
    }
}
